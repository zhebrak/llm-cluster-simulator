...
export const TRAINING_BEGINNER_TASKS: GameTask[] = [
  ...
  // ── Task 3: Activation Memory ───────────────────────────────────────
  {
    ...
    hints: [
      ...
      'Reduce the Micro-Batch Size. Gradient accumulation (GA = GBS / MBS) automatically increases to preserve the same effective batch size with less memory per forward pass.',
    ],
    successExplanation:
  ...
  // ── Task 5: Flash Attention ──────────────────────────────────────────
  {
    ...
    successExplanation:
      ...
      `FA2 eliminates O(N²) HBM traffic for attention scores by tiling the computation in SRAM. ` +
      ...
      `without FA. The memory savings (O(N²) → O(N) activation memory) can be the difference between fitting and OOM ` +
  ...
  // ── Task 7: When Models Don't Fit ──────────────────────────────────
  {
    ...
    briefing:
      ...
      'replicates all of this on every GPU, so adding more GPUs doesn\'t help with memory.\n\n' +
  ...
  // ── Task 9: Going Multi-Node ────────────────────────────────────────
  {
    ...
    successExplanation:
      'Scaling from 8 GPUs (1 node) to 16 GPUs (2 nodes) nearly doubled throughput, though ' +
